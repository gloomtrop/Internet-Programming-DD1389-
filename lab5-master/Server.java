...
public class Server {
    ...
    public Server() {
        try {
            KeyStore ks = KeyStore.getInstance("JKS", "SUN"); //Keystore object ks will represent our keys
            InputStream is = new FileInputStream(new File("/Users/filipstarberg/.keystore"));//Get self-signed certificate that has been created earlier
            ks.load(is,"rootroot".toCharArray()); //Load file into Keystore object. rootroot is the password to the file storing the certificate
            KeyManagerFactory kmf = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm());
            kmf.init(ks,"rootroot".toCharArray());
            KeyManager km[] = kmf.getKeyManagers(); //Key pair stored in array

            SSLContext sslContext=SSLContext.getInstance("TLS"); //Use TLS protocol
            ...
            // The rest is just lab1. Changed Location: http to Location: https
            HashMap<String, User> cookieMap=new HashMap<>();
            while (true) {
                try (SSLSocket socket = (SSLSocket)ss.accept(); //SSLSocket instead of Socket
            ...
            System.err.println(e.getMessage());
            System.err.println("Some error: " + this.port);
            System.exit(1);
        }
    }
...
class User{
    ...
    User(InetAddress ip){
        ...
        user_num=num_users;
        Random rand = new Random();
        correct_guess = rand.nextInt(100)+1;
